...
public class BinarySearchTester {
	...
	private static int binarySearch(ArrayList<Blob> blobs, Blob bKey1, Blob bKey2, int low , int high) {
		int sumB1 = bKey1.getCoolnessFactor();
		int sumB2 = bKey2.getCoolnessFactor();
		int sumKey = sumB1 + sumB2;
		if(low > high) {
			return -(low +1);
		}
		else {
			...
			int pairSum = blobs.get(mid).getCoolnessFactor() + blobs.get(mid+1).getCoolnessFactor();
			if(sumKey < pairSum) {
				return binarySearch(blobs, bKey1, bKey2, low, mid-2);
			}
			else if(sumKey > pairSum) {
				return binarySearch(blobs, bKey1, bKey2, mid+2, high );
			}
			else {
				return mid;
			}
