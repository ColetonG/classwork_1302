...
public class WarehouseTest {
	...
	public static void testAddItem() {
		...
		boolean checked = vod.addItem(sampleItem());
		int exNum = 1;
		int acNum = vod.getNumItems();
		String msg = String.format("Checked = %b\nExpected Num Items: %d "
				+ "Actual Num Items: %d\n",checked, exNum, acNum);
	...
	public static void testGetItemsWithName() {
		...
		Warehouse vod = createWarehouse5();
		String exItem = "Ice";
		String acItem = vod.getItemsWithName(exItem).toString();
		String msg = String.format("Expected items with names containing: %s\n"
				+ "Items returned:\n%s",exItem, acItem);
		System.out.println(msg);
	}
	
	public static void testHasItemTrue() {
		System.out.println("-->testHasItemTrue");
		Warehouse vod = createWarehouse5();
		Item dummy = new Item ( "Ice", 0);
		boolean exR = true;
		boolean acR = vod.hasItem(dummy.getName());
		String msg = String.format("Expected result: %b\n"
				+ "Actual result: %b", exR, acR);
		System.out.println(msg);
	}
	
	public static void testHasItemFalse() {
		System.out.println("-->testHasItemFalse");
		Warehouse vod = createWarehouse5();
		Item dummy = new Item ( "Change", 0);
		boolean exR = false;
		boolean acR = vod.hasItem(dummy.getName());
		String msg = String.format("Expected result: %b\n"
				+ "Actual result: %b", exR, acR);
